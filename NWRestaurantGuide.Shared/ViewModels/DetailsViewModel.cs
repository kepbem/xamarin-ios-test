...
namespace NWRestaurantGuide.Shared.Models
{
	public class DetailsViewModel : BaseViewModel
	{
		...
		const string DetailsQueryUrl = "https://maps.googleapis.com/maps/api/place/details/json?placeid={0}&key=" + RestaurantFilterViewModel.APIKey;
		...
		public string Distance
		{
			get
			{
				string distance = Place.GetDistance(Position.Latitude, Position.Longitude,
					...
					NWRestaurantGuide.Shared.GeolocationUtils.DistanceUnit.Kilometers :
					NWRestaurantGuide.Shared.GeolocationUtils.DistanceUnit.Miles).ToString("##.###", CultureInfo.CurrentUICulture);
		...
		public async Task<bool> RefreshPlace()
		{
			...
			IsBusy = true;
#if !DEBUG
			Xamarin.Insights.Track ("Details", "Restaurant", Place.Name);
#endif
