...
using NWRestaurantGuide.Core.Models;
using RestaurantFilter.UITests.Shared;

namespace NWRestaurantGuide.Core.ViewModels
{
	public class RestaurantFilterViewModel : BaseViewModel
	{
#if DEBUG
...
#endif
		const string RestaurantQueryUrl = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?types=restaurant&location={0},{1}&opennow=true&rankby=distance&key={2}";
		...
		public static string GetDistanceToPlace(Place place, Position position)
		{
			...
			string distance = place.GetDistance(position.Latitude, position.Longitude,
				...
				NWRestaurantGuide.Core.GeolocationUtils.DistanceUnit.Kilometers :
				NWRestaurantGuide.Core.GeolocationUtils.DistanceUnit.Miles).ToString("##.###", CultureInfo.CurrentUICulture);

			return string.Format("{0} {1}", distance, CultureInfo.CurrentCulture.Name != "en-GB" ? "km" : "mi.");
		}
		...
		public async Task<List<Place>> GetPlaces(string query = null)
		{
			...
			{
				requestUri = string.Format(RestaurantQueryUrl,
...
#if !DEBUG
						Xamarin.Insights.Report (ex, "RestaurantViewModel", "GetPlaces");
#endif
					}
				}
			}
		...
		string GetUITestResults()
		{
			...
			{
				case UITestsHelpers.TestType.Closedrestaurant:
				...
				case UITestsHelpers.TestType.OpenRestaurant:
